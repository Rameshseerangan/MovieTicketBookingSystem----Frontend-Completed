...
const PaymentPage = () => {
  ...
  const [isPaymentTypeSelected, setIsPaymentTypeSelected] = useState(false); // To control UI flow
  ...
  const handlePaymentConfirmation = async () => {
    ...
    const paymentData = {
      ...
      ticketInfo: {
        ...
        seats: bookingDetails.seats.map((seat) => seat.toString()), // Keep seats as an array
    ...
    try {
      setLoading(true);
      const response = await axios.post('http://localhost:5000/api/auth/book-ticket', paymentData);

      if (response.status === 200) {
        alert('Payment successful! Confirmation sent to your email.');
        setLoading(false);
        // Optionally clear localStorage and redirect to a success page
        navigate('/');
      }
    } catch (error) {
      setLoading(false);
  ...
  const handlePaymentTypeSelection = (event) => {
    ...
    setPaymentType(type);
    setIsPaymentTypeSelected(true); // Once a payment type is selected, show payment components
  };
    ...
    <div className="min-h-screen bg-gray-900 text-white flex justify-center items-center py-10">
      <div className="bg-gray-800 shadow-xl rounded-lg p-8 w-full max-w-6xl flex space-x-8">
        ...
        <div className="w-1/3 space-y-4 bg-gray-700 p-6 rounded-lg shadow-lg">
          <h2 className="text-2xl font-bold text-center text-indigo-400">Movie Details</h2>
          {bookingDetails ? (
          ...
          <h2 className="text-2xl font-bold text-center text-indigo-400 mt-6">Select Payment Method</h2>
          <select
            value={paymentType}
        ...
        <div className="w-2/3 space-y-6 bg-gray-800 p-6 rounded-lg shadow-xl">
          {isPaymentTypeSelected ? (
            <>
              <div className="mb-4">
                <label className="block text-lg mb-2 text-indigo-400">Email ID:</label>
                <input
                  type="email"
                  value={email}
              ...
              <div className="mb-4">
                <label className="block text-lg mb-2 text-indigo-400">Phone Number:</label>
                <input
                  type="tel"
                  value={phone}
              ...
              <div className="mb-4">
                <label className="block text-lg mb-2 text-indigo-400">Selected Payment: {paymentType}</label>
              </div>
            ...
            <div className="text-center text-lg text-gray-400">Please select a payment method.</div>
          )}
        </div>
      </div>
    </div>
